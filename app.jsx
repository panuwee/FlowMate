...
function App() {
  ...
  useEffectApp(() => {
    ...
    setAutomationAccess({ userId, state: "loading" });
    ...
    Promise.race([request, timeout]).then(result => {
      if (cancelled) return;
      if (result.error) {
        setAutomationAccess({ userId, state: result.error.code === "42501" ? "denied" : "unavailable" });
        return;
      }
      ...
      if (typeof capabilities?.sharedRead !== "boolean" || typeof capabilities?.battlePassRead !== "boolean") {
        setAutomationAccess({ userId, state: "unavailable" });
        return;
      }
      setAutomationAccess({ userId, state: capabilities.sharedRead || capabilities.battlePassRead ? "allowed" : "denied" });
    }).catch(() => {
      if (!cancelled) setAutomationAccess({ userId, state: "unavailable" });
  ...
  async function handleOpenNotification(notification) {
    ...
    try {
      if (window.loadFlowMateWorkItemById) {
        const row = await window.loadFlowMateWorkItemById(notification.workItemId, { includeArchived: true });
        if (row) {
  ...
  useEffectApp(() => {
    ...
    async function refreshNavCounts(event) {
      ...
      if (event?.detail?.reason && window.flowMateRefreshReasonMatches && !window.flowMateRefreshReasonMatches(event, [
        "work_items", "assignment_runs", "work_item_events", "work_status_changed", "admin_work_status_changed",
        "active_team_changed", "recheck_brief", "capacity_allocation_rescheduled", "admin_archive", "admin_restore",
      ])) return;
      try {
  ...
  const allowedRoute = isTaskAssignProduct
    ? TASK_ASSIGN_NAV.flatMap(group => group.items).some(item => item.key === route) || route === "detail"
...
function filterMarketingPlanRowsByFunctions(rows, selectedCodes, campaignCatalogRows) {
  ...
  const campaignByName = new Map((campaignCatalogRows || []).map(campaign => [
    getMarketingPlanCampaignKey(campaign.name),
    String(campaign.functionCode || "").toLowerCase()
  ]));
...
const MARKETING_PLAN_REFRESH_REASONS = [
  ...
  "marketing_plan_working_sheet_time_updated", "marketing_plan_working_sheet_status_updated",
  "marketing_plan_working_sheet_row_edited", "marketing_plan_working_sheet_row_deleted",
