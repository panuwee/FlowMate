...
function BoardScreen({ onOpen, searchQuery = "" }) {
  ...
  function canTransitionBoardWork(row) {
    ...
    if (window.FLOWMATE_ACTIVE_PRODUCT === "task-assign" && row.type === "quick" && currentUser.role !== "admin") {
      const ownFunction = window.normalizeFlowMateWorkspaceTeam
        ? window.normalizeFlowMateWorkspaceTeam(currentUser.requester_team)
        : String(currentUser.requester_team || "").trim().toLowerCase();
