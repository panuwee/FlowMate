...
Local Vitest checks validate source and application contracts only. Before UAT or deployment, an approved staging operator must run `supabase/ot_request.sql` followed by the read-only `supabase/ot_request_verify.sql` and validate PostgreSQL compilation, audit backfill/trigger installation, RLS access, effective grants, reason/consent/note enforcement, deterministic reassignment locks and replay, safe deactivation, and concurrent overlap/weekly accounting behavior. This staging SQL gate is mandatory because local source-contract tests are not database-runtime proof.
