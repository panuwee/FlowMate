...
alter table public.work_items
  drop constraint if exists work_items_effort_for_creative_after_brief;
...
alter table public.assignment_runs
  alter column effort_point drop not null;
...
begin
  ...
  if v_wi.publish_time is null and not v_is_no_tag then v_missing := array_append(v_missing, 'publish time'); end if;
