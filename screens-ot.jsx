...
function OtOwnerInsightsPanel({ refreshKey = 0 }) {
  ...
  return <div className="ot-manager">
    ...
    <section className="ot-analytics-card" aria-label="Confirmed OT trend chart"><div className="ot-section-head"><div><h2>Confirmed OT trend</h2><p className="muted">Actual hours confirmed in each Bangkok week of the selected month.</p></div></div><div className="ot-insight-chart">{weeklyTrend.map(row => <div className="ot-insight-chart__row" key={row.weekStart}><div className="ot-insight-chart__label"><strong>{formatOtDate(row.weekStart)}</strong><span>{formatOtHours(row.actualMinutes)}</span></div><div className="ot-insight-chart__track" aria-label={`${formatOtDate(row.weekStart)}: ${formatOtHours(row.actualMinutes)}`}><span style={{ width: `${Math.round((row.actualMinutes / maxTrendMinutes) * 100)}%` }} /></div></div>)}</div></section>
    <section className="ot-analytics-card" aria-label="Confirmed OT by Function chart"><div className="ot-section-head"><div><h2>Confirmed OT by Function</h2><p className="muted">Compare the confirmed actual-hours share for this month.</p></div></div>{functionRows.length ? <div className="ot-insight-chart">{functionRows.map(row => <div className="ot-insight-chart__row" key={row.value}><div className="ot-insight-chart__label"><strong>{row.label}</strong><span>{formatOtHours(row.actualMinutes)}</span></div><div className="ot-insight-chart__track" aria-label={`${row.label}: ${formatOtHours(row.actualMinutes)}`}><span style={{ width: `${Math.round((row.actualMinutes / maxFunctionMinutes) * 100)}%` }} /></div></div>)}</div> : <div className="ot-state">No confirmed OT is available for this month.</div>}</section>
