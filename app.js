...
async function createMarketingPlanWorkingSheetRow(form) {
  ...
  const content = await window.flowmateSupabase.from("marketing_content_items").insert({
  ...
  const placements = await window.flowmateSupabase.from("marketing_channel_placements").insert(placementRows);
  if (placements.error) throw placements.error;
  return {
    planId,
    campaignId,
    contentItemId: content.data.id,
