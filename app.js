...
function isMarketingPlanNoTagSelection(channels) {
  return Array.isArray(channels) && channels.length === 1 && channels[0] === "no_tag";
}
function getMarketingPlanInlineTimeUi(row, canManageSchedule, updatingRowId) {
  ...
  return {
    ...
    title: isNoTag ? "Not required for No Tag" : canManageSchedule ? "" : "Only PIC, Sub PIC, Admin, or a schedule operator can change Time and Status."
  };
}
...
function groupMarketingPlanWorkingSheetRows(rows, selectedMonth, selectedChannel = "all") {
  ...
  return Array.from(groups.values()).map(group => {
    const sortedPlacements = group.placements.sort((a, b) => String(a.publishDate || "").localeCompare(String(b.publishDate || "")) || String(a.publishTime || "").localeCompare(String(b.publishTime || "")) || String(a.channel || "").localeCompare(String(b.channel || "")));
    ...
    const orderedChannels = MARKETING_PLAN_CHANNELS.map(channel => channel.key).filter(channel => group.channels.includes(channel));
    return {
      ...group,
...
async function findOrCreateMarketingPlan(monthKey) {
  ...
  const existing = await window.flowmateSupabase.from("marketing_plans").select("id").eq("month_key", monthKey).neq("status", "archived").order("created_at", {
    ascending: true
  }).limit(1);
  if (existing.error) throw existing.error;
  if (existing.data && existing.data[0]) return existing.data[0].id;
  const inserted = await window.flowmateSupabase.from("marketing_plans").insert({
    month_key: monthKey,
    title,
    market: "TH",
...
async function findOrCreateMarketingCampaign(planId, name, team) {
  ...
  if (existing.error) throw existing.error;
  if (existing.data && existing.data[0]) return existing.data[0].id;
  const inserted = await window.flowmateSupabase.from("marketing_campaigns").insert({
    plan_id: planId,
    name: campaignName,
    team: team || null,
    start_date: null,
...
async function createMarketingPlanWorkingSheetRow(form) {
  ...
  return {
    contentItemId,
    placementCount: insertRows.length
  };
}
...
async function updateMarketingPlanWorkingSheetPlacementFields(contentItemId, changes) {
  ...
  const result = await window.flowmateSupabase.from("marketing_channel_placements").update(payload).eq("content_item_id", contentItemId);
  if (result.error) throw result.error;
  window.dispatchEvent(new CustomEvent("flowmate:refresh-request", {
    detail: {
...
function getCampaignPlannerWindow(anchor, span = 3) {
  ...
  for (let cursor = new Date(axisStart); cursor <= end; cursor.setUTCDate(cursor.getUTCDate() + 7)) {
    ...
    const week = {
      ...
      days: 7,
      monthStart: owner.weeks.length === 0
    };
  ...
  return {
    ...
    axisStart: axisStart.toISOString().slice(0, 10),
    axisDays: weeks.length * 7
  };
}
...
function validateCampaignPlannerForm(form) {
  ...
  if (Boolean(form.startDate) !== Boolean(form.endDate)) errors.dates = "กรุณาระบุทั้งวันเริ่มและวันสิ้นสุด หรือเว้นว่างทั้งคู่";else if (form.startDate && form.endDate < form.startDate) errors.dates = "วันสิ้นสุดต้องไม่ก่อนวันเริ่ม";
  return errors;
}
...
function MarketingPlanCampaignPlannerScreen({
  user
}) {
  ...
  async function loadItems(id) {
    ...
    try {
      ...
      if (aliveRef.current && generation === loadIdRef.current) setItems(current => ({
        ...current,
        [id]: rows
      }));
    } catch (error) {
      if (aliveRef.current && generation === loadIdRef.current) setItemErrors(current => ({
        ...current,
        [id]: "โหลดงานไม่สำเร็จ"
      }));
    }
  ...
  function openCampaign(campaign, edit = false) {
    ...
    setForm({
      ...
      functionCode: campaign.function_code || "",
  ...
  async function archiveCampaign(campaign) {
    ...
    setSaveError("");
    try {
      if (campaign.is_archived) await restoreMarketingPlanCampaignTag(campaign.campaign_tag_id);else await archiveMarketingPlanCampaignTag(campaign.campaign_tag_id);
      dialogRef.current?.close();
      setSelected(null);
      invalidateMarketingPlanDataCache();
      await refresh();
    } catch (error) {
      setSaveError(error.message || "เปลี่ยนสถานะไม่สำเร็จ");
    } finally {
      setSaving(false);
    }
  }
  const filtered = campaigns.filter(campaign => (includeArchived || !campaign.is_archived) && (functionFilter === "all" || campaign.function_code === functionFilter) && `${campaign.name} ${campaign.tagline}`.toLowerCase().includes(search.trim().toLowerCase()));
  const scheduled = filtered.filter(campaign => getCampaignPlannerBar(campaign.start_date, campaign.end_date, range)).sort((a, b) => a.start_date.localeCompare(b.start_date) || a.name.localeCompare(b.name));
  const unscheduled = filtered.filter(campaign => !campaign.start_date || !campaign.end_date);
  const managementRows = [...filtered].sort((a, b) => managerSort === "oldest" ? String(a.created_at || "").localeCompare(String(b.created_at || "")) : managerSort === "recently-used" ? String(b.last_used_at || "").localeCompare(String(a.last_used_at || "")) : String(b.created_at || "").localeCompare(String(a.created_at || "")));
  ...
  function renderItems(campaign) {
    ...
    if (itemErrors[id]) return React.createElement("div", {
      role: "alert"
    }, itemErrors[id], " ", React.createElement("button", {
      className: "btn btn--secondary",
      onClick: () => loadItems(id)
    }, "ลองอีกครั้ง"));
    if (!items[id]) return React.createElement("p", {
      role: "status"
    }, "กำลังโหลดงาน…");
    if (!items[id].length) return React.createElement("p", {
      className: "muted"
    }, "ยังไม่มีงานในแคมเปญนี้");
    return React.createElement("div", {
      className: "campaign-planner__working-scroll"
    }, React.createElement("table", {
      className: "campaign-planner__working"
    }, React.createElement("thead", null, React.createElement("tr", null, React.createElement("th", {
      scope: "col"
    }, "Product / Event"), React.createElement("th", {
      scope: "col"
      ...
      }, React.createElement("td", null, item.content_title), React.createElement("td", null, row.channels.map(getMarketingPlanChannelLabel).join(", ") || "—"), React.createElement("td", null, row.launchDate ? dateLabel(row.launchDate) : "—"), React.createElement("td", null, row.status ? React.createElement("span", {
  ...
  function campaignName(campaign) {
    return React.createElement(React.Fragment, null, React.createElement("button", {
      className: "campaign-planner__name",
      onClick: () => openCampaign(campaign)
    }, campaign.name), React.createElement("span", {
      className: "muted"
    }, campaign.function_label || "Unassigned", campaign.is_archived ? " · Archived" : ""), React.createElement("span", {
      className: "campaign-planner__progress"
    }, getCampaignPlannerProgress(campaign)), campaign.start_date && React.createElement("span", {
      className: "campaign-planner__progress muted"
    }, dateLabel(campaign.start_date), " – ", dateLabel(campaign.end_date)));
  }
  return React.createElement("section", {
    className: "campaign-planner"
  }, React.createElement("div", {
    className: "page-head"
  }, React.createElement("div", null, React.createElement("h1", null, "Campaign Planner")), React.createElement("div", {
    className: "campaign-planner__actions"
    ...
    name: "plus"
  }), " New Campaign"), React.createElement("button", {
    className: "btn btn--secondary",
    "aria-label": "Refresh",
    title: "Refresh",
    ...
    name: "rerun"
  })))), state.status === "error" && React.createElement("div", {
    className: "reason-box reason-box--need",
    role: "alert"
  }, state.message), state.status === "loading" && React.createElement("p", {
    role: "status"
  }, "กำลังโหลด Campaign Planner…"), state.status === "ready" && React.createElement(React.Fragment, null, React.createElement("div", {
    className: "campaign-planner__toolbar"
  }, React.createElement("div", {
    className: "campaign-planner__actions"
  }, React.createElement("label", null, React.createElement("select", {
    className: "select",
    "aria-label": "มุมมอง",
    value: span,
    ...
    value: 3
  }, "3 เดือน"), React.createElement("option", {
    value: 6
  }, "6 เดือน"), React.createElement("option", {
    value: 12
  }, "ทั้งปี"))), React.createElement("button", {
    className: "btn btn--secondary",
    "aria-label": "ช่วงก่อนหน้า",
    onClick: () => setAnchor(shiftCampaignPlannerWindow(range.start, -span))
  }, React.createElement(Icon, {
    name: "chevron",
    style: {
    ...
    value: anchor,
    onChange: event => {
      if (/^[1-9]\d{3}-\d{2}$/.test(event.target.value)) setAnchor(event.target.value);
    }
  })), React.createElement("button", {
    className: "btn btn--secondary",
    ...
    className: "input",
    type: "search",
    placeholder: "ชื่อแคมเปญหรือ Tagline",
    value: search,
    ...
    value: "all"
  }, "All functions"), functions.map(fn => React.createElement("option", {
    key: fn.code,
    value: fn.code
  }, fn.label)))), React.createElement("label", {
    className: "check-pill"
  }, React.createElement("input", {
    type: "checkbox",
    checked: includeArchived,
    onChange: event => setIncludeArchived(event.target.checked)
  }), " Include archived")), manageMode && canManage ? React.createElement("section", {
    "aria-label": "Manage Campaign",
    className: "campaign-planner__catalog"
  }, React.createElement("h2", null, "Manage Campaign"), React.createElement("label", null, "เรียงลำดับ", React.createElement("select", {
    className: "select",
    "aria-label": "เรียงลำดับแคมเปญ",
    value: managerSort,
    onChange: event => setManagerSort(event.target.value)
  }, React.createElement("option", {
    value: "newest"
    ...
    value: "recently-used"
  }, "Most recently used"))), filtered.length === 0 && React.createElement("p", null, "ไม่พบแคมเปญที่ตรงกับตัวกรอง"), managementRows.map(campaign => React.createElement("div", {
    key: campaign.campaign_tag_id,
    className: "campaign-planner__catalog-row"
  }, React.createElement("div", null, campaignName(campaign)), React.createElement("span", null, dateLabel(campaign.start_date), " – ", dateLabel(campaign.end_date)), React.createElement("button", {
    className: "btn btn--secondary",
    onClick: () => openCampaign(campaign, !campaign.is_archived)
  }, campaign.is_archived ? "รายละเอียด" : "แก้ไข")))) : React.createElement(React.Fragment, null, React.createElement("div", {
    className: "campaign-planner__scroller",
    tabIndex: 0,
    "aria-label": "ช่วงเวลาแคมเปญ เลื่อนแนวนอนได้"
  }, React.createElement("div", {
    className: "campaign-planner__grid",
    style: {
      minWidth: Math.max(860, 290 + range.weeks.length * 28)
    }
  }, React.createElement("div", {
    className: "campaign-planner__row campaign-planner__header"
  }, React.createElement("div", {
    className: "campaign-planner__identity"
  }, "Campaign / ความคืบหน้า"), React.createElement("div", null, React.createElement("div", {
    className: "campaign-planner__months",
    style: {
      gridTemplateColumns: range.months.map(month => `${month.weeks.length}fr`).join(" ")
    }
  }, range.months.map(month => React.createElement("div", {
    key: month.key
  }, month.label))), React.createElement("div", {
    className: "campaign-planner__weeks",
    style: {
      gridTemplateColumns: `repeat(${range.weeks.length}, 1fr)`
    }
  }, range.weeks.map(week => React.createElement("span", {
    key: week.start,
    title: `${dateLabel(week.start)} – ${dateLabel(week.end)}`
  }, React.createElement("span", null, Number(week.start.slice(-2)))))))), scheduled.map(campaign => {
    const bar = getCampaignPlannerBar(campaign.start_date, campaign.end_date, range);
    const colour = getMarketingCampaignFunctionStyle({
      lightBackground: campaign.light_background,
      lightForeground: campaign.light_foreground,
      darkBackground: campaign.dark_background,
      darkForeground: campaign.dark_foreground
    });
    return React.createElement(React.Fragment, {
      key: campaign.campaign_tag_id
    }, React.createElement("div", {
      className: "campaign-planner__row"
    }, React.createElement("div", {
      className: "campaign-planner__identity"
    }, React.createElement("button", {
      className: "iconbtn",
      "aria-label": `${expanded.includes(campaign.campaign_tag_id) ? "ยุบ" : "ขยาย"}งาน ${campaign.name}`,
      "aria-expanded": expanded.includes(campaign.campaign_tag_id),
      onClick: () => toggleItems(campaign.campaign_tag_id)
    }, React.createElement(Icon, {
      name: "chevron",
      style: {
        transform: expanded.includes(campaign.campaign_tag_id) ? "rotate(90deg)" : "none"
      }
    })), React.createElement("div", null, campaignName(campaign))), React.createElement("div", {
      ...
      className: "campaign-planner__bar",
      "data-compact": bar.width < 3,
      style: {
        ...colour,
        left: `${bar.left}%`,
        width: `${bar.width}%`
      },
      title: `${campaign.name}: ${dateLabel(campaign.start_date)} – ${dateLabel(campaign.end_date)}`,
      "aria-label": `${campaign.name} ${dateLabel(campaign.start_date)} ถึง ${dateLabel(campaign.end_date)}${bar.before ? " เริ่มก่อนช่วงที่แสดง" : ""}${bar.after ? " ต่อเนื่องหลังช่วงที่แสดง" : ""}`,
      onClick: () => openCampaign(campaign)
    }, bar.before && React.createElement(Icon, {
      name: "chevron",
      size: 12,
      style: {
        transform: "rotate(180deg)"
    ...
    className: "campaign-planner__unscheduled"
  }, React.createElement("h2", null, "รอกำหนดช่วงเวลา ", React.createElement("span", {
    className: "muted"
  }, "(", unscheduled.length, ")")), unscheduled.map(campaign => React.createElement("div", {
    className: "campaign-planner__catalog-row",
    key: campaign.campaign_tag_id
  }, React.createElement("div", null, campaignName(campaign)), canManage && !campaign.is_archived && React.createElement("button", {
    className: "btn btn--secondary",
    onClick: () => openCampaign(campaign, true)
  }, "กำหนดช่วงเวลา")))))), selected && React.createElement("dialog", {
    className: "campaign-planner__dialog",
    ref: dialogRef,
    onCancel: event => {
      if (saving) event.preventDefault();else setSelected(null);
    },
    onClose: () => {
      if (!saving) setSelected(null);
    },
    "aria-labelledby": "campaign-planner-dialog-title"
  }, React.createElement("div", {
    className: "campaign-planner__dialog-head"
  }, React.createElement("h2", {
    id: "campaign-planner-dialog-title"
  }, editing ? selected.campaign_tag_id ? "แก้ไขแคมเปญ" : "New Campaign" : selected.name), React.createElement("button", {
    className: "iconbtn",
    "aria-label": "ปิดรายละเอียดแคมเปญ",
    disabled: saving,
    onClick: closeDialog
  }, React.createElement(Icon, {
    name: "x"
    ...
    role: "alert",
    className: "reason-box reason-box--need"
  }, saveError), editing ? React.createElement("form", {
    onSubmit: saveCampaign,
    noValidate: true
  }, React.createElement("fieldset", {
    disabled: saving,
    className: "campaign-planner__form"
  }, React.createElement("label", null, "ชื่อแคมเปญ", React.createElement("input", {
    autoFocus: true,
    name: "name",
    className: "input",
    value: form.name,
    "aria-invalid": Boolean(errors.name),
    "aria-describedby": "planner-name-help",
    onChange: event => setForm({
      ...form,
      name: event.target.value
    })
  }), React.createElement("span", {
    id: "planner-name-help",
    className: errors.name ? "campaign-planner__error" : "muted"
  }, errors.name || "ตัวอย่าง Summer Sale [Jul-2026]")), React.createElement("label", null, "Tagline (optional)", React.createElement("textarea", {
    name: "tagline",
    className: "input",
    maxLength: 300,
    rows: 2,
  ...
  }), errors.tagline && React.createElement("span", {
    role: "alert"
  }, errors.tagline)), React.createElement("label", null, "Function Colour Tag", React.createElement("select", {
    name: "functionCode",
    className: "select",
    "aria-label": "Function Colour Tag",
    "aria-invalid": Boolean(errors.functionCode),
    "aria-describedby": errors.functionCode ? "planner-function-error" : undefined,
    ...
    value: ""
  }, "เลือก Function"), functions.map(fn => React.createElement("option", {
    value: fn.code,
    key: fn.code
  }, fn.label))), errors.functionCode && React.createElement("span", {
    id: "planner-function-error",
    className: "campaign-planner__error"
  }, errors.functionCode)), React.createElement("div", {
    className: "campaign-planner__dates"
  }, React.createElement("label", null, "วันเริ่มกิจกรรม", React.createElement("input", {
    name: "startDate",
    className: "input",
    type: "date",
    "aria-invalid": Boolean(errors.dates),
    "aria-describedby": errors.dates ? "planner-dates-error" : undefined,
    value: form.startDate,
    onChange: event => setForm({
      ...form,
      startDate: event.target.value
    })
  })), React.createElement("label", null, "วันสิ้นสุดกิจกรรม", React.createElement("input", {
    className: "input",
    type: "date",
    "aria-invalid": Boolean(errors.dates),
    "aria-describedby": errors.dates ? "planner-dates-error" : undefined,
  ...
  }))), errors.dates && React.createElement("p", {
    id: "planner-dates-error",
    role: "alert",
    className: "campaign-planner__error"
  }, errors.dates), React.createElement("div", {
    className: "campaign-planner__actions"
  }, React.createElement("button", {
    className: "btn btn--primary",
    type: "submit"
  }, saving ? "กำลังบันทึก…" : "บันทึกแคมเปญ"), React.createElement("button", {
    type: "button",
    className: "btn btn--secondary",
    onClick: closeDialog
  }, "ยกเลิก")))) : React.createElement(React.Fragment, null, React.createElement("p", null, selected.tagline || "ยังไม่มี Tagline"), React.createElement("p", null, React.createElement("strong", null, "ช่วงกิจกรรม:"), " ", dateLabel(selected.start_date), " – ", dateLabel(selected.end_date)), React.createElement("p", null, React.createElement("strong", null, "ช่วงเผยแพร่จากงาน:"), " ", dateLabel(selected.first_publish_date), " – ", dateLabel(selected.last_publish_date)), React.createElement("p", null, getCampaignPlannerProgress(selected), " · ยกเลิก ", selected.cancelled_items || 0, " งาน"), canManage && !selected.is_archived && React.createElement("button", {
    className: "btn btn--primary",
    disabled: saving,
    onClick: () => setEditing(true)
  }, "แก้ไขแคมเปญ"), React.createElement("h3", null, "งานย่อย"), renderItems(selected)), isAdmin && selected.campaign_tag_id && React.createElement("div", {
    className: "campaign-planner__archive"
  }, React.createElement("button", {
    className: "btn btn--secondary",
    disabled: saving,
    onClick: () => archiveCampaign(selected)
  }, selected.is_archived ? "Restore Campaign" : "Archive Campaign"))));
}
function MarketingPlanTimelineScreen({
  ...
  function renderPlacementBadge(placement) {
    ...
    return React.createElement("div", {
      ...
      className: "marketing-timeline-badge__channel"
    }, channelAbbrev), React.createElement("span", {
  ...
  }, "Campaign / Product Event"), React.createElement("div", {
    style: {
      display: "grid",
      gridTemplateColumns: timelineWindow.monthGroups.map(group => `${group.days.length * columnWidth}px`).join(" ")
    }
  }, timelineWindow.monthGroups.map(group => React.createElement("div", {
    key: group.key,
    className: "eyebrow",
    style: {
      minHeight: 32,
...
function MarketingPlanWorkingSheetScreen() {
  ...
  async function handleSaveWorkingSheetRow(event) {
    ...
    try {
      ...
      setSaveState({
        status: "saved",
        message: `Saved Marketing Plan row with ${result.placementCount} channel placement${result.placementCount === 1 ? "" : "s"}.`
      });
      setSheetForm(current => ({
      ...
      setSaveState({
        status: "error",
        message: window.flowmateUserError ? window.flowmateUserError(error, "Working Sheet save failed.") : "Working Sheet save failed."
      });
    }
  }
  ...
  return React.createElement("div", null, React.createElement("div", {
  ...
  }, "SQL required: supabase/marketing_plan.sql. Saving requires a user role allowed by Marketing Plan RLS.")), React.createElement("div", {
    className: "card",
    style: {
      marginBottom: 16
    }
    ...
    className: "card__title"
  }, "Add monthly working row"), React.createElement("div", {
    className: "card__sub"
  }, "Use this instead of entering the recurring plan in Google Sheet."))), React.createElement("form", {
    className: "card__body",
    onSubmit: handleSaveWorkingSheetRow
  }, React.createElement("div", {
    className: "form-grid"
  }, React.createElement("label", {
    className: "field"
  ...
  }, "Campaign *"), React.createElement("input", {
    className: "input",
    list: "marketing-plan-campaign-tags",
    value: sheetForm.campaignName,
    onChange: event => updateSheetForm("campaignName", event.target.value),
    placeholder: "e.g. New Patch update : 26.05"
  }), React.createElement("datalist", {
    id: "marketing-plan-campaign-tags"
    ...
    className: "field__label"
  }, "Product / Event *"), React.createElement("input", {
    className: "input",
    value: sheetForm.productEvent,
    onChange: event => updateSheetForm("productEvent", event.target.value),
    ...
    className: "select",
    value: sheetForm.assetType,
    onChange: event => updateSheetForm("assetType", event.target.value)
    ...
    className: "field__label"
  }, "Content Tier *"), React.createElement("select", {
    className: "select",
    value: sheetForm.contentTier,
    onChange: event => updateSheetForm("contentTier", event.target.value)
    ...
    className: "textarea",
    rows: "3",
    value: sheetForm.details,
    onChange: event => updateSheetForm("details", event.target.value),
    placeholder: "Content details, message, format requirements, or sheet note."
  })), React.createElement("div", {
    className: "field field--full"
  }, React.createElement("span", {
    className: "field__label"
    ...
    className: "check-row"
  }, MARKETING_PLAN_CHANNELS.filter(channel => channel.key !== "other").map(channel => React.createElement("label", {
    key: channel.key,
    className: "check-pill"
  }, React.createElement("input", {
    type: "checkbox",
  ...
  }), React.createElement("span", null, channel.label))))), React.createElement("label", {
    className: "field field--full"
  }, React.createElement("span", {
    className: "field__label"
    ...
    className: "textarea",
    rows: "2",
    value: sheetForm.note,
    onChange: event => updateSheetForm("note", event.target.value),
    placeholder: "Internal note, dependency, posting instruction, or campaign context."
  }))), saveState.message && React.createElement("div", {
    className: `reason-box ${saveState.status === "error" ? "reason-box--need" : ""}`,
    style: {
      marginTop: 12
    }
  }, saveState.message), React.createElement("div", {
    ...
    type: "button",
    className: "btn btn--secondary",
    onClick: () => setSheetForm(getDefaultMarketingPlanWorkingSheetForm())
  }, "Clear"), React.createElement("button", {
    type: "submit",
    className: "btn btn--primary",
    disabled: saveState.status === "saving"
  }, saveState.status === "saving" ? "Saving..." : "Save to Marketing Plan")))), React.createElement("div", {
    className: "card"
  }, React.createElement("div", {
    className: "card__head"
  }, React.createElement("div", null, React.createElement("span", {
    ...
    className: "table table--dense marketing-working-table"
  }, React.createElement("thead", null, React.createElement("tr", null, React.createElement("th", {
    className: "col-month"
...
function MarketingPlanShell({
  ...
  }, activeSection.key === "campaign-planner" && React.createElement(MarketingPlanCampaignPlannerScreen, {
    user
...
function GlobalSearchResultsPanel({
  ...
  return React.createElement("div", {
    ...
    className: "searchbar-results__head"
  }, React.createElement("span", null, "Search results"), React.createElement("span", {
    className: "mono"
  }, safeResults.length)), loadState && loadState.status === "error" && React.createElement("div", {
    className: "searchbar-results__empty"
  }, loadState.message), loadState && loadState.status !== "error" && safeResults.length === 0 && React.createElement("div", {
    className: "searchbar-results__empty"
  }, "No results for \"", query, "\"."), safeResults.map(row => React.createElement("button", {
    key: row.id,
    type: "button",
    ...
    className: "searchbar__result-main"
  }, React.createElement("span", {
    className: "searchbar__result-id mono"
...
function NotificationCenterPanel({
  ...
  return React.createElement("div", {
    ...
    className: "card__title"
  }, "Notification Center"), React.createElement("div", {
    className: "card__sub"
  }, unreadCount, " unread")), React.createElement("div", {
    className: "row",
    style: {
      gap: 6
    }
  ...
  }, safeNotifications.length, " notifications"), React.createElement("button", {
    ...
    name: "check",
    size: 11
  }), " Mark all as read"), React.createElement("button", {
    className: "btn btn--xs btn--ghost",
  ...
  }, "Clear read")), hasError && React.createElement("div", {
    className: "reason-box reason-box--need",
    style: {
      marginBottom: 10
    }
  }, loadState.message), isLoading && safeNotifications.length === 0 && React.createElement("div", {
    className: "reason-box"
  }, "Loading notifications..."), !isLoading && safeNotifications.length === 0 && React.createElement("div", {
    className: "reason-box",
    style: {
      textAlign: "center"
    }
    ...
    className: "strong"
  }, "No notifications yet"), React.createElement("div", {
    className: "muted",
    style: {
      marginTop: 4,
      fontSize: 12
    }
  }, "Assignments, review requests, comments, links, watchers, and due reminders will appear here.")), safeNotifications.length > 0 && React.createElement("div", {
    style: listStyle
    ...
    return React.createElement("div", {
      ...
      className: `badge ${isUnread ? "badge--overdue" : "badge--neutral"}`
    }, isUnread ? "Unread" : "Read"), React.createElement("span", {
      className: "tag"
    }, notificationKindLabel(notification.type)), notification.workItemId && React.createElement("span", {
      className: "mono muted",
      style: {
        fontSize: 11
      }
    }, notification.workItemId)), React.createElement("div", {
      className: "strong",
      style: {
        overflowWrap: "anywhere",
        wordBreak: "break-word"
      }
    }, notification.title), notification.body && React.createElement("div", {
      className: "muted",
      style: {
        fontSize: 12,
        marginTop: 3,
    ...
    }, notification.body), React.createElement("div", {
      className: "muted",
      style: {
        fontSize: 11,
        marginTop: 6
      }
    }, notification.createdLabel || "No timestamp")), isUnread && React.createElement("button", {
      className: "btn btn--xs btn--ghost",
      onClick: event => {
        event.stopPropagation();
...
function CreateMenuPanel({
  ...
  return React.createElement("div", {
    className: "topbar__create-menu",
    role: "menu"
  }, React.createElement("div", {
    className: "topbar__create-menu-head"
  }, React.createElement("span", null, "Create new"), React.createElement("button", {
    type: "button",
    className: "iconbtn",
    onClick: onClose,
    ...
    className: "topbar__create-option-title"
  }, React.createElement(Icon, {
    name: "zap",
    size: 14
    ...
    name: "layers",
    size: 14
  }), " Creative Request"), React.createElement("span", {
    className: "topbar__create-option-sub"
  }, "Brief for assignment engine")), React.createElement("button", {
    type: "button",
    className: "topbar__create-option",
    role: "menuitem",
    onClick: onLeave
    ...
    name: "calendar",
    size: 14
  }), " Leave request"), React.createElement("span", {
    className: "topbar__create-option-sub"
  }, "Date range shown on calendar")));
}
function GlobalLeaveRequestModal({
  onClose
}) {
  const todayKey = flowMateTodayDateKey();
  const [leaveForm, setLeaveForm] = useStateApp({
    startDate: todayKey,
    endDate: todayKey,
    startHalf: "am",
    endHalf: "pm",
  ...
  const [leaveState, setLeaveState] = useStateApp({
    status: "idle",
    message: ""
  });
  async function submitLeaveRequest(event) {
    ...
    if (!window.createFlowMateLeaveRequest) {
      setLeaveState({
        status: "error",
        message: "Leave request helper is not ready."
      });
      return;
    }
    setLeaveState({
...
function AccessDeniedScreen({
  ...
  return React.createElement("div", {
    ...
    className: "reason-box reason-box--need"
  }, "This page is only available to FlowMate admins."), React.createElement("div", {
    style: {
      marginTop: 12
    }
  }, React.createElement("button", {
...
function LiveStatus({
  ...
  useEffectApp(() => {
    ...
    return () => {
      window.removeEventListener("flowmate:synced", onSynced);
      clearInterval(id);
    };
  }, []);
  useEffectApp(() => {
  ...
  const realtimeMessage = realtimeStatus === "connected" ? "Realtime connected" : realtimeStatus === "syncing" ? "Realtime syncing" : realtimeStatus === "degraded" ? "Realtime degraded" : "Polling fallback active";
  const dotColor = realtimeStatus === "connected" ? "var(--garena-positive)" : realtimeStatus === "syncing" ? "var(--garena-orange)" : realtimeStatus === "degraded" ? "var(--garena-red)" : "var(--garena-grey)";
  void tick;
  return React.createElement("div", {
    style: {
      padding: "16px 24px",
  ...
  }), React.createElement("span", null, realtimeMessage)), React.createElement("div", {
    className: "row",
    style: {
      gap: 6,
      fontSize: 12,
  ...
  }, React.createElement("span", null, "Synced ", label)));
}
...
function LoginScreen({
  ...
  useEffectApp(() => {
    ...
    function spawn() {
      ...
      const anim = orb.animate([{
        ...
        opacity: 0.95,
        offset: 0.08,
        transform: `translate(${drift * 0.1}px, -8vh)`
      }, {
        opacity: 0.78,
        offset: 0.55,
        transform: `translate(${drift * 0.6}px, -60vh)`
      }, {
        opacity: 0,
        transform: `translate(${drift}px, -115vh)`
      }], {
        duration: duration,
        easing: "ease-out",
        fill: "forwards"
  ...
  return React.createElement("div", {
    className: "wha"
  }, React.createElement("style", null, WHA_STYLES), React.createElement("div", {
    id: "wha-orb-stage",
    className: "wha-orb-stage",
    "aria-hidden": "true"
  }), React.createElement("main", {
    className: "wha-hero",
    id: "threshold"
  }, React.createElement("div", {
    className: "wha-sigil-stage",
    "aria-hidden": "true"
  }, React.createElement("svg", {
    className: "wha-glass",
    viewBox: "0 0 500 500"
  }, React.createElement("g", null, React.createElement("polygon", {
    points: "250,90 380,170 380,330 250,410 120,330 120,170",
    fill: "var(--glass-blue)",
    stroke: "var(--ink)",
    strokeWidth: "2",
    strokeLinejoin: "round"
  }), React.createElement("polygon", {
    points: "250,130 390,250 250,370 110,250",
    fill: "var(--glass-teal)",
    stroke: "var(--ink)",
    strokeWidth: "2",
    strokeLinejoin: "round"
  }), React.createElement("polygon", {
    className: "wha-glass__amber",
    points: "250,180 340,250 250,320 160,250",
    fill: "var(--glass-amber)",
    stroke: "var(--ink)",
    strokeWidth: "2",
    strokeLinejoin: "round"
  ...
  }), [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map(deg => React.createElement("line", {
    key: deg,
    x1: "250",
    y1: "55",
    x2: "250",
  ...
  })), [15, 75, 105, 165, 195, 255, 285, 345].map(deg => React.createElement("circle", {
    key: deg,
    cx: "250",
    cy: "70",
    r: "2.4",
    ...
    className: "wha-sigil__glyphs",
    "aria-hidden": "true"
  }, [0, 45, 90, 135, 180, 225, 270, 315].map((angle, i) => React.createElement("g", {
    key: angle,
    transform: `translate(250 250) rotate(${angle}) translate(0 -165)`
  }, React.createElement(SigilGlyph, {
    kind: i
  }))))), React.createElement("g", {
    ...
    d: "M 250,230 L 264,250 L 250,270 L 236,250 Z",
    fill: "var(--sienna)",
    stroke: "var(--ink)",
    strokeWidth: "1"
  }), React.createElement("circle", {
  ...
  switch (kind % 8) {
    ...
    case 5:
      return React.createElement("g", {
        ...
        d: "M 0,-9 L 5.29,7.28 L -8.56,-2.81 L 8.56,-2.81 L -5.29,7.28 Z"
      }));
    case 6:
      return React.createElement("g", {
        fill: "none",
        ...
        d: "M -10,0 Q 0,-7 10,0 Q 0,7 -10,0 Z"
      }), React.createElement("circle", {
        cx: "0",
        cy: "0",
        r: "2.6",
    ...
    default:
      return React.createElement("g", {
        ...
        d: "M 5,-9 Q -5,-5 -5,0 Q -5,5 5,9 Q -1,5 -1,0 Q -1,-5 5,-9 Z",
        fill: stroke
      }));
  }
}
function InkDivider() {
  return React.createElement("svg", {
    ...
    points: "600,11 612,20 600,29 588,20",
    fill: "var(--sienna)",
    stroke: "var(--ink)",
    strokeWidth: "0.8"
  }), React.createElement("circle", {
...
function CardGlyph({
  ...
  if (kind === "quill") {
    return React.createElement("svg", {
      ...
      d: "M 30,30 Q 42,16 52,8 Q 54,22 42,32 Q 35,33 30,30 Z",
      fill: "currentColor",
      stroke: "currentColor",
      strokeWidth: "0.5"
    }), React.createElement("path", {
      d: "M 32,32 L 36,28 M 36,30 L 40,26 M 40,28 L 44,24",
      stroke: "var(--cream)",
      strokeWidth: "0.6"
    }), React.createElement("line", {
      x1: "14",
  ...
  if (kind === "glass") {
    return React.createElement("svg", {
      ...
      d: "M 15,8 L 45,8 L 30,30 L 45,52 L 15,52 L 30,30 Z",
      fill: "none",
      stroke: "currentColor",
      strokeWidth: "1.4",
      strokeLinejoin: "round"
    }), React.createElement("path", {
      d: "M 22,14 L 38,14 L 30,25 Z",
      fill: "currentColor",
      opacity: "0.45"
    }), React.createElement("path", {
      d: "M 22,46 L 38,46 L 30,35 Z",
      fill: "currentColor",
      opacity: "0.2"
    }), React.createElement("circle", {
      cx: "30",
  ...
  return React.createElement("svg", {
    ...
    d: "M 30,10 L 36,30 L 30,26 L 24,30 Z",
    fill: "currentColor"
  }), React.createElement("path", {
    d: "M 30,50 L 24,30 L 30,34 L 36,30 Z",
    fill: "currentColor",
    opacity: "0.55"
  }), React.createElement("circle", {
    cx: "30",
...
function ShelfRow() {
  function Herb({
    ...
    return React.createElement("g", {
      ...
      fill: "var(--forest)",
      opacity: "0.85",
      transform: "rotate(20 5 54)",
      stroke: "var(--ink)",
      strokeWidth: "0.4"
    }), React.createElement("path", {
      d: "M -3,60 Q 0,68 3,60",
      fill: "none",
      stroke: "var(--sienna)",
      strokeWidth: "0.8"
    }));
  ...
  function Potion({
    ...
    return React.createElement("g", {
      ...
      d: "M -5,16 L -5,30 Q -16,38 -14,55 Q -10,68 0,68 Q 10,68 14,55 Q 16,38 5,30 L 5,16 Z",
      fill: "var(--glass-teal)",
      stroke: "var(--ink)",
      strokeWidth: "1"
    }), React.createElement("path", {
      d: "M -10,48 Q -6,42 -2,46 Q 2,50 6,46 Q 10,42 12,48",
      fill: "none",
      stroke: "var(--ink)",
      strokeWidth: "0.5",
      opacity: "0.5"
  ...
  function Quill({
    ...
    return React.createElement("g", {
      ...
      d: "M 4,26 Q 16,14 22,6 Q 24,18 16,28 Q 9,30 4,26 Z",
      fill: "var(--ink)",
      stroke: "var(--ink)",
      strokeWidth: "0.4"
    }), React.createElement("path", {
      d: "M 6,28 L 10,24 M 10,26 L 14,22 M 14,24 L 18,20",
      stroke: "var(--cream)",
      strokeWidth: "0.5"
    }), React.createElement("circle", {
      cx: "-16",
  ...
  return React.createElement("svg", {
    ...
    stroke: "var(--ink)",
    strokeWidth: "1.2"
  }), React.createElement(Herb, {
    x: 100
  }), React.createElement(Herb, {
    x: 220
  }), React.createElement(Potion, {
    x: 370
  }), React.createElement(Herb, {
    x: 520
  }), React.createElement(Quill, {
    x: 660
  }), React.createElement(Herb, {
    x: 800
  }), React.createElement(Potion, {
...
window.flowmatePrompt = function (opts) {
  return new Promise(function (resolve) {
    ...
    try {
      window.dispatchEvent(new CustomEvent("flowmate:prompt-open", {
        detail: opts || {}
      }));
    } catch (e) {
...
function FlowMatePromptHost() {
  ...
  const modalStyle = {
    background: "var(--garena-white, #fff)",
    color: "var(--garena-iron, #2b2f36)",
    width: "100%",
    maxWidth: 460,
    borderRadius: 10,
  ...
  const inputStyle = {
    ...
    padding: "9px 11px",
    fontSize: 14,
    border: "1px solid var(--garena-light-grey, #d8dce3)",
    borderRadius: 6,
    fontFamily: "inherit"
  };
  return React.createElement("div", {
    style: overlayStyle,
    onMouseDown: () => settle(null)
  }, React.createElement("div", {
    role: "dialog",
    "aria-modal": "true",
    style: modalStyle,
    ...
    onSubmit: onSubmit
  }, req.title ? React.createElement("div", {
    style: {
      fontSize: 16,
      fontWeight: 700,
      marginBottom: 10
    }
  }, req.title) : null, req.label ? React.createElement("label", {
    style: {
      fontSize: 13,
      fontWeight: 600,
      display: "block"
    ...
    style: {
      color: "var(--garena-red, #c0504d)"
    }
  }, " *") : null) : null, !req.hideInput && (req.multiline ? React.createElement("textarea", {
    autoFocus: true,
    rows: 4,
    style: inputStyle,
    value: value,
    ...
    style: {
      fontSize: 12,
      color: "var(--garena-grey, #6b7280)",
      marginTop: 8,
      lineHeight: 1.5
    }
  }, req.note) : null, error ? React.createElement("div", {
    style: {
      fontSize: 12.5,
      color: "var(--garena-red, #c0504d)",
      marginTop: 8
